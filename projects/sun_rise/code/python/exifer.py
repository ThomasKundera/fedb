...
class ExifHandler:
    ...
    def __init__(self, directory, time_ref, threshold_minutes=5):
        """
        Initialize the ExifHandler for the base directory (e.g., 'data').

        :param directory: Base directory containing subdirs like '<some_ref>'.
        :param time_ref: List of tuples (filename, provided_correct_timestamp), where timestamp is a datetime.datetime object.
    ...
    def process_subdirs(self):
        ...
        for camera in refs_by_camera:
            ...
            deltas_camera = []
            if group_avgs:
                # First interval: -inf to first avg_exif + midpoint to next
    ...
    def get_camera_key(self, imgfile):
        """Get camera model from EXIF (fallback, since ID not retrieved)."""
        try:
            image = Image.open(imgfile)
            exif_data = image._getexif()
            image.close()
            if exif_data:
                return exif_data.get(0x0110, 'Unknown')
        except Exception as e:
    ...
    def fix_all_images(self, fix=False):
        """Fix timestamps for all images in subdirs of the directory (not time_ref or IMG subdirs)."""
        for subdir in os.listdir(self.directory):
            subdir_path = os.path.join(self.directory, subdir)
            if os.path.isdir(subdir_path):
                ...
                if os.path.isdir(img_dir):
                    for img in sorted(os.listdir(img_dir)):
                        if img.lower().endswith('.jpg'):
                            ...
                            corrected_ts = self.fix_timestamp(imgfile, fix=fix)
                            if corrected_ts:
                                logprint(f"Corrected {imgfile} to {corrected_ts}")